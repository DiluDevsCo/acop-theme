...
if ( ! class_exists( 'acf_roles_chooser' ) ) :

	class acf_roles_chooser extends acf_field_select {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name          = 'acf_roles_chooser';
      $this->label         = _x( 'Roles', 'noun', 'acf' );
			...
			// ajax
			add_action( 'wp_ajax_acf/fields/acf_roles_chooser/query', array( $this, 'ajax_query' ) );
			add_action( 'wp_ajax_nopriv_acf/fields/acf_roles_chooser/query', array( $this, 'ajax_query' ) );
      // add_filter('acf/pre_render_fields', array($this,'pre_render_fields'),10,2);
		}

    // function pre_render_field($field, $post_id) {
    //   $field['type'] = 'select';
    //   return $field;
    // }
    ...
    // }
      ...
      wp_enqueue_script( 'acf-roles-chooser', get_stylesheet_directory_uri() . '/assets/acf-roles-chooser.js', array('jquery', 'select2'), '', true   );
      ...
      return $choices;
		...
		*/
		...
		function render_field( $field ) {
			...
			$choices = acf_get_array( $this->get_choices());
		...
		*/
		...
		public function validate_rest_value( $valid, $value, $field ) {
			...
			$allowed = empty( $option_keys ) ? $choices : $option_keys;
		...
		public function get_rest_schema( array $field ) {
			...
			$schema = array(
				...
				'items'    => array(
					...
					'enum' => empty( $option_keys ) ? $choices : $option_keys,
				),
			);
	...
	// initialize
	acf_register_field_type( 'acf_roles_chooser' );
